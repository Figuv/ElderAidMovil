...
const CampaignsScreen = (props) => {
  ...
  //     query(collection(db, "campaign"), where("status", "==", type ? 1 : 0)),
  //     (snapshot) => {
  //       const campaignList = [];
    ...
    <SafeAreaView className="bg-white">
      <View className="items-center mb-32">
        <View className="flex-row justify-center my-2 p-1 space-x-2 rounded-full">
          <TouchableOpacity
            className={`bg-${type ? "black" : "white"} px-2 py-1 rounded-full justify-center items-center`}
            onPress={() => setType(true)}
          >
            <Text className="text-3xl font-extrabold text-secondary">OnGoing</Text>
          </TouchableOpacity>
          <TouchableOpacity
            className={`bg-${type ? "white" : "black"} px-2 py-1 rounded-full justify-center items-center`}
            onPress={() => setType(false)}
          >
            <Text className="text-3xl font-extrabold text-secondary">PastDue</Text>
          </TouchableOpacity>
        </View>
